...
function intersection(a) {
    if (a.length > 2) return intersection([intersection(a.slice(0, a.length / 2)), intersection(a.slice(a.length / 2))]);
...
let keys =[ ...foundSolution.keys() ].sort();
let solution = []
for (let k of keys) {
	solution.push(foundSolution.get(k))
}
